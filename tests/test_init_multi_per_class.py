...
def test_env_initialisation(test_env):
    ...
    assert test_env.num_agents == 3, "Number of agents not initialized correctly."
...
def test_env_reset(test_env):
    ...
    assert len(state.agents) == 3, "Environment did not initialize correct number of agents on reset."
    ...
    assert state.agents[2].species_id == 1, "Agent 2 species ID not initialized correctly on reset."

    assert state.adj.shape == (3, 3), "Adjacency matrix the wrong shape."
...
def test_create_adj_matrix_fc_interclass(test_env):
    ...
    assert state.adj.shape == (3, 3), "Adjacency matrix not the correct shape."
    assert (state.adj == jnp.array(
        [[0., 1., 1.],
         [1., 0., 0.],
         [1., 0., 0.]]
