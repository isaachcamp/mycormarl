...
Generate the complete heterogeneous solution-P field at reset. The field must:

- be non-negative;
- be spatially correlated in physical `(r, z)` coordinates;
- preserve the exact configured total initial labile-P inventory within the
  initially P-bearing region;
- preserve the volume-weighted mean concentration;
- use cell-volume weights for realized mean, variance, and coefficient of
  variation;
- leave cells outside the configured P-bearing region at zero; and
