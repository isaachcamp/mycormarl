...
- **Child ticket**: a GitHub sub-issue, falling back to a task-list entry and `Part of #<map>` when sub-issues are unavailable.
- **Blocking**: GitHub’s native issue dependencies, falling back to a `Blocked by: #<n>` line.
