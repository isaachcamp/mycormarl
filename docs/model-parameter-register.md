...
|---|---:|---|---|
| `initial_biomass` | `0.001 g DM` | Plant dry biomass at reset; immediately determines starting root length, photosynthetic input, maintenance demand, and death reference. | The repository previously described this as a rounded `1.05 mg` observation, but its cited DOI resolves to an unrelated shrub-steppe water-uptake paper and cannot support the claim. Valid *D. carota* population means span `0.7–3.3 mg` ([Vandelook et al. 2024](https://doi.org/10.1017/S0960258524000230)), so `1 mg` is plausible. **Literature-bounded model choice, not currently direct-backed.** |
| `initial_c_pool` | `0.000402 g C` | Free, allocatable carbon at reset; this is additional to structural C implicit in biomass. | One structural-biomass equivalent: `0.001 × 0.402`. **Derived model initial condition.** |
| `initial_p_pool` | `0.00192 mg P` | Free, allocatable phosphorus at reset; additional to structural P implicit in biomass. | One structural-biomass equivalent: `0.001 × 1.92`. **Derived model initial condition.** |
| `kleaf` | `0.30` | Fraction of whole-plant dry biomass contributing to photosynthesis; carbon input is directly proportional to it. | No quantitative source is currently recorded. It is compatible with, but not derived by, the separate `kroot=0.62`; the remaining `0.08` is implicit non-root, non-photosynthetic biomass. **Unsupported.** |
| `kroot` | `0.62` | Root dry-mass fraction; multiplies whole-plant biomass before SRL converts mass to total root length. | Rounded from the median `0.6198` across eight *D. carota* root-mass-fraction study-site entries in GRooT ([Guerrero-Ramírez et al. 2021](https://doi.org/10.1111/geb.13179)). Standing allocation is used as a proxy for marginal growth allocation. **Derived.** |
...
| `root_radius` | `0.01 cm` (`100 µm`) | Converts root length to absorbing lateral area and enters cylindrical sparse-uptake resistance. | No empirical source or root-order definition is recorded. **Unsupported provisional value.** |
| `specific_root_length` | `25,434.3 cm g⁻¹ root DM` (`254.343 m g⁻¹`) | Converts root dry mass to total absorbing root length; uptake geometry scales linearly with it until domain clipping. | Median of ten *D. carota* SRL study-site entries in GRooT, separate from the records used for `kroot` ([Guerrero-Ramírez et al. 2021](https://doi.org/10.1111/geb.13179)). **Derived; unmatched trait aggregation.** |
...
| Runtime field | Default and units | Description and model effect | Evidence, derivation, and status |
|---|---:|---|---|
| `initial_biomass` | `7.97e-7 g DM` | Fungal dry biomass at reset. The model assumes the symbiosis is already established and maps the entire proxy mass to living external absorptive hyphae; it does not simulate a dormant spore. | Sieverding et al.'s five-species counted-and-dried AMF regression, `M_µg = 0.4458e-5 d_µm^2.5372`, evaluated at the `117.5 µm` midpoint of the reported *R. irregularis* `70–165 µm` range ([Sieverding et al. 1989](https://doi.org/10.1016/0038-0717(89)90013-8); [INVAM species description](https://invam.ku.edu/irregularis)). It predicts `0.797 µg`. **Cross-species derived estimate, not a measured *R. irregularis* mean.** |
| `initial_c_pool` | `3.985e-7 g C` | Free fungal C at reset, additional to structural C implicit in biomass. | One structural-biomass equivalent: `7.97e-7 × 0.5`. **Derived model initial condition.** |
| `initial_p_pool` | `1.594e-6 mg P` | Free fungal P at reset, additional to structural P implicit in biomass. | One structural-biomass equivalent: `7.97e-7 × 2`. **Derived model initial condition.** |
...
| `dt` | `0.025 d` (`0.6 h`) | Biological/action step and basis for maintenance, photosynthesis, uptake, and termination timing. Diffusion is internally subcycled if required. | Finest candidate in the existing qualification after coarser coupled cases failed the 5% endpoint rule; no finer reference proved convergence. **Numerical fallback, not a biological parameter estimate.** |
