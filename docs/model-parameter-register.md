...
| `initial_c_pool` | Derived at reset: `3.75e-8 g C` at `dt=0.025 d` | Free fungal C at reset, additional to structural C implicit in biomass. | When unspecified, exactly one next-step maintenance requirement: `kappa_c × initial_biomass × dt`. A supplied numeric value is an explicit override. **Derived model initial condition.** |
| `initial_p_pool` | Derived at reset: `2.5e-9 mg P` at `dt=0.025 d` | Free fungal P at reset, additional to structural P implicit in biomass. | When unspecified, exactly one next-step maintenance requirement: `kappa_p × initial_biomass × dt`. A supplied numeric value is an explicit override. **Derived model initial condition.** |
...
| `kappa_p` | `0.001 mg P g⁻¹ DM d⁻¹` | Lumped irreversible free-P loss, excluding P transfer to the plant and structural P in biomass. | Set equal to the plant value so both partners carry the same small free-P-loss burden. This is an explicit model-policy choice, not a fungal measurement. Literature constrains fine-hyphal lifetimes to roughly `5–7 d` ([Bago et al. 1998](https://doi.org/10.1046/j.1469-8137.1998.00199.x); [Olsson & Johnson 2005](https://doi.org/10.1111/j.1461-0248.2005.00831.x)), but not the non-recycled P fraction. With `gamma_p=1.3`, it implies about `0.38–0.54%` irreversible loss per fine-structure turnover: `kappa_p=(gamma_p/tau)f_irreversible`. **Model choice; not directly literature-backed.** |
